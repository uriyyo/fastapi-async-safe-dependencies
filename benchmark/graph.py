...
async def main() -> None:
    concurrency = [1, *range(5, 101, 5)]
    results = [await benchmark(requests=5_000, concurrency=c) for c in concurrency]
    ...
    diff_plt.set_xlim(min(concurrency), max(concurrency))
    ...
    mean_plt.set_xlim(min(concurrency), max(concurrency))
    mean_plt.legend()
